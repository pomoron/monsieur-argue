...
REPO_ROOT = os.path.dirname(ASSESSOR_DIR)
NEGOTIATOR_DIR = os.path.join(REPO_ROOT, "monsieur-argue-main")
...
def run_session(engine, persona_name, max_rounds, *, script=None, color=True):
    ...
    print(_wrap(engine.opening_statement()))
    print("─" * 74)
...
def play_once(*, scenario_path, persona_path, norms_path, playbook_path,
    ...
    config = build_config(model=model, max_rounds=max_rounds)
    engine = engine_mod.NegotiationEngine(
        config=config, company_norms=norms, scenario=scenario_d, persona=persona,
    ...
    print(c.bold(c.cyan(
        ...
        f"{'[MOCK opponent]' if neg_mock else ''} ══")))
    if learnings_used:
        print(c.dim("  (opponent sharpened against your past weaknesses)"))

    reason, detail = run_session(engine, persona["name"], max_rounds, script=script, color=color)
